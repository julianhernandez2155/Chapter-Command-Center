-- Structured member housing verification.
-- Members choose a housing type first, then fill the relevant detail:
-- dorm/building for on-campus, street address for off-campus, or chapter
-- house/building plus address for chapter housing.
...
comment on column public.members.campus_housing is 'Dorm/building name for on-campus members, apartment/building name for off-campus members, or chapter house/building for chapter housing.';
...
select
  ...
  array_remove(array[
    ...
    case when m.housing_type in ('off_campus', 'chapter_housing') and nullif(trim(coalesce(m.local_address, '')), '') is null then 'local_address' end,
